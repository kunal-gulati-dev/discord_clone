...
## Chat Item Component.
1. So for that we have to create a new component named <ChatItem /> and replace this code.
```
{group.items.map((message: MessageWithMemberWithProfile) => (
    // <div key={message.id}>
    //     {message.content}
...
```
2. So create a new component in components/chat/chat-item.tsx file.
3. we are including timestamp on the chat messages so we have to add a package called date-fns.
4. npm install date-fns.
5. updated chat-item component props given below.
```
{group.items.map((message: MessageWithMemberWithProfile) => (
    // <div key={message.id}>
    //     {message.content}
...
```
6. chat-item.tsx file code till now because after that we have to add cases for pdf and images.
```
"use client"
...
```
7. Updated code without Edit and Delete functionality.
```
"use client"
...
```
8. Now we have to add the edit and delete functionality, So we have to create a feature when clicked on Edit it should open the intut with the current message.
9. So till now we have created the edit functionaly, we imported alot of form and input, axios etc and after this we have to create route for submitting the form.
code till now of chat-item.tsx file.
```
"use client"
...
```
10. create a folder in pages/api/socket/messages. and shift the file named messages.tsx and renamed it to index.ts and create a file named [messageId].ts in it also.
11. The complete code for above mentioned file is given below.
```
import { currentProfilePages } from "@/lib/current-profile-page";
import { db } from "@/lib/db";
import { NextApiResponseServerIo } from "@/types";
...
```
12. After this we have check that if it is working or not, So it will work but not directly we have to refresh the page to see it because we have not applied the websocket to the frontend.
13. also make these two changes in the onSubmit function.
```
form.reset();
setIsEditing(false)
```
...
6. create a hook named use-chat-socket.ts in hooks folder and write below mentioned code.
...
```
7. Now we have to use these socket in chat-messages.tsx file add below mentioned code.
```
